"""
Enhanced Domain Models - Updated with New Fields and States

Updates:
- Enhanced BotInfo with channel_secondary and bio fields
- New SessionState values for expanded Set Info
- Import fix for JSON usage in services
"""
...
class SessionState(str, enum.Enum):
    ...
    SET_INFO_CHANNEL_SECONDARY = "set_info_channel_secondary"  # New
    SET_INFO_BIO = "set_info_bio"  # New
...
# ==========================================
# ENHANCED DOMAIN DATACLASSES (DTOs)
# ==========================================

@dataclass
class BotInfo:
    """Enhanced bot information with new fields"""
    name: Optional[str] = None
    channel: Optional[str] = None
    group: Optional[str] = None
    welcome_message: Optional[str] = None
    contact: Optional[str] = None
    channel_secondary: Optional[str] = None  # New field
    bio: Optional[str] = None  # New field
    
    def format_info_message(self) -> str:
        """Format bot info for /info command"""
        lines = []
        
        if self.name:
            lines.append(f"🏷️ **Nazwa:** {self.name}")
        
        if self.bio:
            lines.append(f"📝 **Opis:** {self.bio}")
        
        if self.channel:
            lines.append(f"📺 **Kanał:** {self.channel}")
            
        if self.channel_secondary:
            lines.append(f"📺 **Kanał 2:** {self.channel_secondary}")
        
        if self.group:
            lines.append(f"👥 **Grupa:** {self.group}")
        
        if not lines:
            lines.append("ℹ️ **Informacje o bocie**")
            lines.append("Skonfiguruj informacje w panelu administratora.")
        
        return "\n".join(lines)
    
    def format_contact_message(self) -> str:
        """Format contact info for /kontakt command"""
        if self.contact:
            return f"📞 **Kontakt**\n\n{self.contact}"
        else:
