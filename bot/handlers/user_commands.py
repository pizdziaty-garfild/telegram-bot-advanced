...
/help - Ta pomoc"""
    ...
    async def handle_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            if awaiting == 'groups_add':
                ...
                async with command_bus.database.get_session() as db:
                    await db.execute(text(
                        """
                        INSERT INTO config (key, value, encrypted, created_at, updated_at)
                        VALUES ('info_text', :val, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                ...
                async with command_bus.database.get_session() as db:
                    await db.execute(text(
                        """
                        INSERT INTO config (key, value, encrypted, created_at, updated_at)
                        VALUES ('kontakt_text', :val, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                        ON CONFLICT(key) DO UPDATE SET value=:val, updated_at=CURRENT_TIMESTAMP
                        """
                    ), {"val": text_input})
                    await db.commit()
                ...
                try:
                    ...
                    async with command_bus.database.get_session() as db:
                        await db.execute(text(
                            """
                            INSERT INTO config (key, value, encrypted, created_at, updated_at)
                            VALUES ('global_interval_minutes', :val, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                            ON CONFLICT(key) DO UPDATE SET value=:val, updated_at=CURRENT_TIMESTAMP
                            """
                        ), {"val": str(minutes)})
                        await db.commit()
                    ...
                    await update.effective_chat.send_message(f"Nieprawidłowa liczba: {e}")
                    return  # Don't clear awaiting, let user retry
                except Exception as e:
                    await update.effective_chat.send_message(f"Błąd zapisu: {e}")
                    return
                ...
                try:
                    ...
                    async with command_bus.database.get_session() as db:
                        await db.execute(text(
                            """
                            INSERT INTO config (key, value, encrypted, created_at, updated_at)
                            VALUES ('excluded_interval_minutes', :val, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                            ON CONFLICT(key) DO UPDATE SET value=:val, updated_at=CURRENT_TIMESTAMP
                            """
                        ), {"val": str(minutes)})
                        await db.commit()
            ...
            # Wyczyść stan oczekiwania po powodzeniu
    ...
    # Handler do odbierania inputów w trybie admin - musi być ostatni
